...
def inference_video(opt):
    ...
    for fidx, fname in enumerate(flist):
        ...
        if len(bboxes) == 1:
            dets.append([]);
            dets[-1].append({'frame': fidx, 'bbox': (bboxes[0][:-1]).tolist(), 'conf': bboxes[0][-1]})
            print('%05d conf: %s' % (fidx, bboxes[0][-1]))
...
for shot in scene:
    ...
    if shot[1].frame_num - shot[0].frame_num >= opt.min_track:
        ...
        # print('Start:' + str(shot[0].frame_num) + '，end：' + str(shot[1].frame_num) +', 共 ' + str(shot[1].frame_num - shot[0].frame_num))
